...
module type Set_and_Map = sig
  ...
  val add_set: unit -> unit -> elt -> set -> unit * set
  val singleton: elt -> set
  val is_singleton: set -> bool 
  val remove: unit ->unit -> elt -> set -> unit * set 
  val union: unit ->unit -> set -> set -> unit * set 
  val inter: unit ->unit -> set -> set -> unit * set
  val diff: unit ->unit -> set -> set -> unit * set 
  ...
  val filter: unit ->unit -> (elt -> bool) -> set -> unit * set
  val partition: unit ->unit -> (elt -> bool) -> set -> unit * set * set
  val cardinal: set -> int
  val elements: set -> elt list
  val min_elt: set -> elt 
  val max_elt: set -> elt 
  val choose: set -> elt
  val split: unit ->unit -> elt -> set -> unit * (set * bool * set)
  val empty_map: 'a map
  val is_empty_map: 'a map -> bool
  val add_map: unit ->unit -> key -> 'a -> 'a map -> unit * 'a map
  val find_map: unit ->unit -> key -> 'a map -> unit * 'a
  val find_map_option : unit ->unit -> key -> 'a map -> unit * 'a option 
  val remove_map: unit ->unit -> key -> 'a map -> unit * 'a map
  ...
  val update_map: unit ->unit -> 'a map -> 'a map -> unit * 'a map    
  val map2_map: unit ->unit -> ('a -> 'a -> 'a) -> 'a map -> 'a map -> unit * 'a map 
  val fold2z_map: unit -> unit -> (key -> 'a  -> 'b  -> (unit * 'c)  -> (unit * 'c)) -> 'a map -> 'b map -> 'c -> unit * 'c 
   val fold2_map: unit -> unit -> (key -> 'a  -> 'b  -> (unit * 'c)  -> (unit * 'c)) -> (key -> 'a   -> (unit * 'c)  -> (unit * 'c)) -> (key -> 'b  -> (unit * 'c)  -> (unit * 'c)) ->  'a map -> 'b map -> 'c -> unit * 'c 
  val forall_map: (key -> 'a -> bool) -> 'a map -> bool 
  val min_elt_map: (key -> 'a -> bool) -> 'a map -> key option  
  val diff_map: unit ->unit -> 'a map -> 'a map -> unit * 'a map * 'a map 
  val diff_map_pred: unit ->unit -> ('a -> 'a -> bool) -> 'a map -> 'a map -> unit * 'a map * 'a map 
end
...
module Make(Ord:OrderedType) =  
  ...
  let rec find_map parameters rh key map = 
    match map with  
          | Empty_map -> (*Exception.warn parameters rh (Some "Set_and_map.ml") (Some "find_map, line 405") Not_found (fun () -> raise Not_found)*) raise Not_found 
